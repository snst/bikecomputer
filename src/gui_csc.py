...
class GuiCsc(GuiBase):
    ...
    def show_id(self, data):
        if self.shown_data.id != data.id:
            self.main.text(fonts.middle, "%d"%data.id, 0, Display.height-fonts.middle.HEIGHT)
    ...
    def show_speed_avg(self, data, y):
        if self.shown_data.speed_avg != data.speed_avg:
            ...
            self.show_float_speed(data.speed_avg, 75, y, color = col)
    ...
    def show_speed_final(self, goal, y):
        self.show_float_speed(goal.calc_required_average_km_h, 0, y, color=Color.blue)

    def show_distance_goal(self, data, y):
        self.show_float_speed(data.remaining_distance_km, 0, y)
    ...
    def show(self, redraw_all):
        ...
        y = 0
        y_goal = fonts.giant.HEIGHT - fonts.huge.HEIGHT
        y_1 = 65
        y_2 = y_1 + 60
        y_3 = y_2 + 50
